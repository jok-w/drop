...
def read_engine_metadata(path: Path) -> dict[str, Any]:
    with path.open("rb") as stream:
        ...
        if not 0 < size <= min(1024 * 1024, path.stat().st_size - 4):
            raise ValueError(f"engine 缺少项目元数据，请使用 tracking.export_model 重新构建：{path}")
...
def validate_source(metadata: dict, source_hash: str) -> dict:
    ...
    if metadata.get("task") != "detect" or not metadata.get("names"):
        raise ValueError("导出模型缺少目标检测类别信息")
    if metadata.get("end2end") or tracking.get("postprocess") != "external-nms":
        raise ValueError("导出检测头与外部 NMS 策略不一致，请重新导出")
    return tracking
