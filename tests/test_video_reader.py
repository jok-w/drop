...
from tracking.gst_decoder_worker import copy_bgrx, send_message
from tracking.video_reader import (
...
class ReaderTests(unittest.TestCase):
    ...
    def test_auto_fallback_and_forced_decoder(self):
        capture = Mock()
        with patch('tracking.video_reader.cv2.VideoCapture', return_value=capture), \
                patch('tracking.video_reader.is_jetson', return_value=False), \
                patch('tracking.video_reader.check_decoder') as probe:
        ...
        with patch('tracking.video_reader.cv2.VideoCapture', return_value=capture), \
                patch('tracking.video_reader.is_jetson', return_value=True), \
                patch('tracking.video_reader.check_decoder', side_effect=RuntimeError('GI missing')):
            self.assertIs(create_video_capture(Path('in.mp4')), capture)
    ...
    def test_worker_accepts_mpeg4_parser(self):
        from tracking import gst_decoder_worker as worker
        gst = Mock()
            ...
            for decoder in ('auto', 'gstreamer'):
                for error in (RuntimeError('startup failed'), OSError('startup failed')):
                    with self.subTest(stage=stage, decoder=decoder, error=type(error)):
                        ...
                        with patch('tracking.video_reader.cv2.VideoCapture', return_value=capture), \
                                patch('tracking.video_reader.is_jetson', return_value=True), \
                                patch('tracking.video_reader.check_decoder'), \
                                patch('tracking.video_reader.GStreamerVideoCapture',
                                      return_value=reader,
    ...
    def test_successful_startup_commits_to_nvdec_and_later_errors_propagate(self):
        ...
        with patch('tracking.video_reader.cv2.VideoCapture', return_value=capture), \
                patch('tracking.video_reader.is_jetson', return_value=True), \
                patch('tracking.video_reader.check_decoder') as check, \
                patch('tracking.video_reader.GStreamerVideoCapture', return_value=reader) as factory:
