...
from tracking.video_writer import (
...
class VideoWriterTests(unittest.TestCase):
    def make_writer(self, folder, timeout=30):
        ...
        with patch("tracking.video_writer.subprocess.Popen", return_value=process), \
                patch("tracking.video_writer.os.set_blocking", create=True):
    ...
    def test_partial_pipe_writes_preserve_every_pixel_and_frame(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            try:
                with patch("tracking.video_writer.select.select", return_value=([], [123], [])), \
                        patch("tracking.video_writer.os.write", side_effect=write_chunk):
                    writer.write(frame)
                    writer.write(frame)
    ...
    def test_broken_pipe_aborts_without_counting_frame(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            with patch("tracking.video_writer.select.select", return_value=([], [123], [])), \
                    patch("tracking.video_writer.os.write", side_effect=BrokenPipeError):
    ...
    def test_stalled_pipe_times_out(self):
        with tempfile.TemporaryDirectory() as folder:
            writer, process = self.make_writer(folder)
            with patch("tracking.video_writer.time.monotonic", side_effect=[0, 31]):
    ...
    def test_metadata_mismatch_is_not_reported_as_success(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            with patch("tracking.video_writer.cv2.VideoCapture", return_value=capture):
    ...
    def test_auto_uses_opencv_off_jetson_and_falls_back_only_during_probe(self):
        ...
        with patch("tracking.video_writer.is_jetson", return_value=False), \
                patch("tracking.video_writer.check_gstreamer") as probe, \
                patch("tracking.video_writer.cv2.VideoWriter") as cv_writer:
            self.assertIs(create_video_writer(*args, "auto", "mp4v", 8000000), cv_writer.return_value)
            probe.assert_not_called()
        with patch("tracking.video_writer.is_jetson", return_value=True), \
                patch("tracking.video_writer.check_gstreamer", side_effect=RuntimeError("missing")), \
                patch("tracking.video_writer.cv2.VideoWriter") as cv_writer:
            self.assertIs(create_video_writer(*args, "auto", "mp4v", 8000000), cv_writer.return_value)
            with self.assertRaisesRegex(RuntimeError, "missing"):
                create_video_writer(*args, "gstreamer", "mp4v", 8000000)
        with patch("tracking.video_writer.is_jetson", return_value=True), \
                patch("tracking.video_writer.check_gstreamer", return_value="gst"), \
                patch("tracking.video_writer.GStreamerVideoWriter", side_effect=OSError("launch failed")), \
                patch("tracking.video_writer.cv2.VideoWriter") as cv_writer:
